...
//        } else { rs = null; }
     ...
     //             return_array.push({intersect:intersect_points, object:node});
                  intersect_points = _intersect_E(ray, node.i);
                  if( intersect_points != false ) {
                  var tmin = (intersect_points[0].a-ray[0].a)/ray[0].b;
                    if( !return_node ) {
                        return_array.push({intersect:tmin, object:node.o});
                    } else {
                        return_array.push({intersect:tmin, object:node});
                    }
                  }
              }
        }
